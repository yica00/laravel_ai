...
@section('content')
    ...
    <div class="main-wrap fadeInRight">
        ...
        <h3><a href="/admin/level/create" >新增等级</a></h3>
        <table class="tablePanel2" cellpadding="0" cellspacing="0" width="100%">
            ...
            @foreach($levels as $sort)
                <tr style="border-bottom:1px solid #BBB;height: 40px">

                    <td>{{$sort->id}}</td>
                    <td>{{$sort->name}}</td>
                    <td>
                        <a class="act-btn" href="/admin/level/{{$sort->id}}/edit">编辑</a>
                        <a class="act-btn" onclick="if(confirm('确定删除此项？')){location.href='/admin/level/{{$sort->id}}/delete'}"} >删除</a>
                    </td>
                </tr>
            @endforeach
